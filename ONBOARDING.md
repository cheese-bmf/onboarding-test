...
여기서부터는 **b.stage 계정·대상 스테이지·조직·콘솔**이 필요하다. Phase 1의 로컬 렌더가 성공했더라도 배포 대상이 없으면 배포 완료로 판정하지 않는다. 아래 순서대로 상태를 확인하고, 각 관문은 사용자의 준비 작업과 에이전트의 재검증이 모두 통과해야 다음 단계로 넘어간다.
...
- **사용자가 준비할 것**:
  1. b.stage에서 사용할 대상 스테이지를 만들거나, 담당자에게 스테이지 ID/URL을 요청한다.
  2. G3 GitHub 조직 접근은 승인에 시간이 걸리므로 스테이지 준비와 동시에 먼저 신청한다.
  3. API를 사용하는 화면이면 G2 자격증명을 준비하고, 순수 화면이면 G2를 건너뛸지 결정한다.
  4. 스테이지가 준비된 뒤 G4 Console 활성화를 요청할 담당자와 커스텀 URL을 확인한다.
- **어디서 확인하는지**: 스테이지 ID/URL은 b.stage 계정 또는 담당자가 안내한 스테이지 관리 화면에서, 조직 접근은 GitHub 조직과 배포 리포에서, Console 활성화는 b.stage Console에서 확인한다. 확인되지 않은 스테이지나 URL을 추측해 배포하지 않는다.
- **왜 필요한지**: 스테이지가 없으면 G2 API 인증의 대상, G4 활성화할 URL, 배포 후 실제 화면을 확인할 위치가 모두 정해지지 않는다.
- **완료 판정**: 사용할 스테이지 ID/URL이 정해지고, 사용자가 해당 스테이지에 접근할 수 있으며, G3 신청 상태와 G2/G4 준비 상태를 확인할 수 있어야 한다. 이 정보가 없으면 Phase 2는 계속 `WAITING_STAGE`다.
- **재개 흐름**: 사용자가 준비를 마치고 **“완료했어”**라고 말하면 에이전트는 스테이지 ID/URL의 접근 가능 여부와 현재 로컬 프로젝트 위치를 다시 확인한다. 대상이 확인되면 G2→G3→G4 순서로 이어가고, 여전히 대상이 없거나 접근할 수 없으면 확인된 오류와 다음 준비 작업을 안내한 뒤 이 CHECKPOINT에서 대기한다.
...
- **사용자가 준비·실행할 것**: 파트너 콘솔에서 `appId`와 `appSecret`을 발급받아 프로젝트의 `src/shared/client.ts`에 설정한다. `appId`는 `bsa_...`, `appSecret`은 `bsp_...` 형식이어야 하며, 토큰·Secret 원문은 대화나 로그에 붙여 넣지 않는다.
- **어디서 확인하는지**: 발급 상태는 파트너 콘솔에서, 설정 위치와 형식은 프로젝트의 `src/shared/client.ts`에서 확인한다. `bsa_`와 `bsp_`가 서로 바뀌지 않았는지 확인한다.
...
- **재개 흐름**: 사용자가 설정 후 **“완료했어”**라고 말하면 에이전트는 `client.ts`의 키 존재 여부·접두사·역할을 비밀값 없이 점검하고, 문서나 SDK에 정의된 실제 인증 확인 경로가 있으면 그것을 실행한다. 검증이 통과하면 G3로 넘어가고, 실패하면 원문 인증 오류와 수정할 위치를 안내한 뒤 G2에서 다시 대기한다.

### ⛔ WAITING G3 — GitHub 조직 접근 (배포 리포)
- **현재 상태**: 배포 리포가 사설 GitHub 조직에 있거나, 조직 멤버십이 아직 승인되지 않았다. 승인 전에는 배포용 리포를 찾거나 push하려고 시도하지 않는다.
- **사용자가 준비·실행할 것**: flex 기안으로 배포 조직(`partners-bmf` 등)의 접근 권한을 신청하고, 승인 후 포털에 표시된 배포 리포 URL을 확인한다. 승인 전이라면 이 신청부터 진행한다.
- **어디서 확인하는지**: 권한 승인 상태는 flex/조직 담당자에게, 실제 접근은 GitHub에서 해당 조직과 배포 리포를 열어 확인한다. 리포 링크가 404이면 멤버십 또는 리포 URL이 아직 해결되지 않은 것이다.
...
- **재개 흐름**: 사용자가 승인 후 **“완료했어”**라고 말하면 에이전트는 포털에 표시된 리포 URL을 기준으로 GitHub 접근을 재검증한다. 리포 접근이 확인되면 G4로 넘어가고, 404/권한 오류가 계속되면 원문 오류와 필요한 권한을 안내한 뒤 G3에서 다시 대기한다.
...
- **사용자가 준비·실행할 것**: 내부 담당자에게 대상 스테이지, 커스텀 템플릿명/URL, 배포 환경을 전달하고 Console에서 해당 템플릿/URL을 **활성화 ON**으로 요청한다. 활성화 전에는 배포 성공 여부만으로 완료 처리하지 않는다.
- **어디서 확인하는지**: 활성화 상태는 b.stage Console에서, 실제 라우팅 결과는 대상 스테이지의 커스텀 URL에서 확인한다. Console에 활성화 상태가 보이지 않거나 담당자 확인이 필요하면 그 사실을 CHECKPOINT로 남긴다.
...
- **재개 흐름**: 사용자가 담당자 확인 후 **“완료했어”**라고 말하면 에이전트는 Console의 활성화 상태와 대상 URL 응답/화면을 다시 확인한다. 정상 화면이면 배포 완료로 이동하고, 404·미노출·릴리즈 미적용이면 원인과 다음 조치를 안내한 뒤 G4에서 다시 대기한다.
