...
여기서부터는 **b.stage 계정·조직·콘솔**이 필요하다. 대상 스테이지가 없으면 대부분 CHECKPOINT이며, 그 자체가 정상이다(로컬까지는 이미 성공). 아래 G2~G4는 각각 CHECKPOINT다 — 위 "진행 방식"대로 안내하고 재개 신호("완료했어")를 받으면 해당 관문의 확인 명령을 다시 돌려 통과 시 다음으로 이어간다. 여러 관문이 한 번에 필요하면 사용자가 처리할 것들을 함께 안내해도 된다.
