...
2. 그 문서의 지시를 **그대로** 따른다:
   ...
   - **끝내지 말고 이어간다(체크포인트-재개 루프)**: 자동 단계는 멈추지 말고 쭉 진행하고, 사람 관문(PARTNERS_TOKEN·API 키·GitHub 조직·Console 활성화)에서는 무엇을·어디서·왜를 안내한 뒤 사용자의 "완료했어" 신호를 기다렸다가, 신호를 받으면 그 관문의 확인 명령을 다시 돌려 통과 시 다음 단계로 이어간다. "온보딩 중단"으로 세션을 끝내지 않는다
